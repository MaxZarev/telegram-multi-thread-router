...
def _build_orchestrator_system_prompt(provider: str) -> str:
    ...
    return (
        ...
        "  IMPORTANT: When the user asks to create a schedule, ONLY call create_schedule. "
        "Do NOT create sessions, send messages to sessions, or perform any other side actions. "
        "If the user says 'first run now' or 'start immediately', set the cron so the first "
        "trigger is soon — do NOT manually run the task yourself.\n"
